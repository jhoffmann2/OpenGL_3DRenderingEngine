...
int MainScene::Init()
{
...

    {
        ...
        for (size_t i = 1; i < LightSystem::lightCount; ++i)
        {
            ...
            lightObj->AddComponent(new TransformComponent(
                {0, 0, 0},
                {3.7, 0, 0},
                {EY, 0},
                3.f
    ...
    {
        objects_.emplace_back(new GameObject("Environment"));
        ...
        RenderingComponent *rendering = new RenderingComponent(
            sphere_mesh,
            SolidRender::FLAT_EMISSION
        );
        rendering->SetDiffuseTexture(Texture(
            "../../Common/textures/Environment/14-Hamarikyu_Bridge_B_3k.hdr"));
...
int MainScene::Render()
{
    ...
    if (ImGui::Begin("Camera"))
    {
        ...
        direction = glm::rotate(direction, glm::radians(camPolarY),
            glm::cross(glm::vec3{direction}, glm::vec3{0, 1, 0}));

        direction *= camRadius;
        cam = Camera{direction, -direction, EY, camFOV, static_cast<float>(viewportWidth_) / viewportHeight_, 0.1f,
            1000};
    ...
    if (ImGui::BeginMainMenuBar())
    {
        ...
        if (ImGui::BeginMenu("LightConversion"))
        {
            ...
            for (size_t i = 0; i < lights_.size(); ++i)
            {
                ...
                ImGui::SameLine();
                ImGui::RadioButton(("Global##" + light->Name()).c_str(), &isGlobal, 1);
                ImGui::SameLine();
    ...
    glm::mat4 NDCToWorld = inverse(
        ShaderGlobalSystem::GetCamToNDC() * transpose(inverse(ShaderGlobalSystem::GetWorldToCam())));
