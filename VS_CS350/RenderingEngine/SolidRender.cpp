...
written consent of DigiPen Institute of Technology is prohibited. File Name:
PhongRender.cpp Purpose: 3D lighting renderer Language: MSVC c++17 Platform:
win32 compiled in Visual Studio 2019 Project: jordan.h_CS300_1 Author: Jordan
Hoffmann, jordan.h Creation date: 9/28/2020 End Header
--------------------------------------------------------*/
...
int SolidRender::loadMesh(const Mesh &m)
{
  ...
  GLHelper::BindVertexAttribute(normal_buffer, MeshData::NORM,
                                m.vertex_normals);
...
void SolidRender::draw(int mi, Texture &diffuseTex, Texture &specularTex)
{
  ...
  glBindVertexArray(vao);
  glLineWidth(3.0f);
  glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(3 * face_count),
                 GL_UNSIGNED_INT, nullptr);
  glBindVertexArray(0);
}

void SolidRender::drawDeferredSolid(int mi)
{
  glUseProgram(GetProgram());
  ...
  GBuffer::RenderSolid(vao, face_count);
}
...
{
  programs_[PHONG_LIGHTING] =
      ::LoadShaders("../../Common/shaders/Forward/PhongLighting.vert",
                    "../../Common/shaders/Forward/SolidLighting.frag");
  programs_[PHONG_SHADING] =
      ::LoadShaders("../../Common/shaders/Forward/SolidShading.vert",
                    "../../Common/shaders/Forward/PhongShading.frag");
  programs_[BLINN_LIGHTING] =
      ::LoadShaders("../../Common/shaders/Forward/BlinnLighting.vert",
                    "../../Common/shaders/Forward/SolidLighting.frag");
  programs_[BLINN_SHADING] =
      ::LoadShaders("../../Common/shaders/Forward/SolidShading.vert",
                    "../../Common/shaders/Forward/BlinnShading.frag");
  programs_[FLAT_EMISSION] =
      ::LoadShaders("../../Common/shaders/Forward/FlatEmission.vert",
                    "../../Common/shaders/Forward/FlatEmission.frag");
  programs_[DEFFERED] =
      ::LoadShaders("../../Common/shaders/Forward/GBuffer.vert",
