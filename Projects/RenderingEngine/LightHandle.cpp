...
const glm::vec3& LightHandle::GetPosition() const
{
  return getData().position_;
}

glm::vec3 LightHandle::GetDirection() const
{
...
{
  Light& data = getData();
  glm::vec3& currentPos = data.position_;
  if(currentPos != pos)
  {
    currentPos = pos;
...
void LightHandle::SetDirection(const glm::vec3& dir)
{
  Light& data = getData();
  glm::vec3& currentDir = data.direction_;
  if (currentDir != -dir)
  {
    currentDir = -dir;
    if (data.active_) LightSystem::SetDirty(index_);
